...
public class SPARQLRunner {
    ...
    private Query getQuery(String filename) {
        ...
        if (!queryCache.containsKey(filename)) {
            String resourcePath = "/queries/" + (subfolder == null ? "" : subfolder + "/") + filename;
            try {
                InputStream in = SPARQLRunner.class.getResourceAsStream(resourcePath);
                if (in == null) {
                    throw new RuntimeException("SPARQL query not on classpath: " + resourcePath
                            + " (run createQueries.sh to generate queries/rdf2es/ when using subfolder rdf2es)");
                }
                return QueryFactory.create(FileUtils.readWholeFileAsUTF8(in));
            } catch (IOException ex) {
                throw new RuntimeException("Failed to read SPARQL query: " + resourcePath, ex);
            }
        }
